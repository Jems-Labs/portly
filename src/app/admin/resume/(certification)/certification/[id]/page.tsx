...
function Certification() {
    ...
    const { fetchCertificate, editCertificate } = useApp();
    ...
    const { data } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetchCertificate(id as string);
            return res;
        },
        staleTime: 1000 * 60 * 3,
    });
    ...
    useEffect(() => {
        if (data) {
            setFormData({
                ...
                issueYear: data.issueYear.toString(),
                expirationMonth: data.expirationMonth,
                expirationYear: data.expirationYear.toString(),
            })
        }
    }, [data])
    ...
    const handleSave = async () => {
        setIsLoading(true);
        await editCertificate(formData, id as string);
        setIsLoading(false);
    }
    ...
    return (
        <div className="px-5">
            <div className="flex items-center gap-4 py-3">
                ...
                <h1 className="text-lg font-bold">Edit Certification</h1>
            </div>
            <Separator />
